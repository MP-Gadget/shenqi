...
create_gadget_parameter_set()
{
    ...
    param_declare_int   (ps, "DomainOverDecompositionFactor", OPTIONAL, 4, "Create on average this number of sub domains on a MPI rank. Higher numbers improve the tree build at the cost of the tree walk.");
    ...
    param_declare_double(ps, "MaxWindFreeTravelTime", OPTIONAL, 60, "Maximum time for the wind to be decoupled, in Myr/h: the internal time unit carries a factor of 1/h, like the internal length and mass units.");
