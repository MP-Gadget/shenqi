...
BOOST_AUTO_TEST_CASE(test_read_no_rescale)
{
    ...
    BOOST_TEST(pow(DeltaSpec(1.124995061548053968e-02/1e3, DELTA_TOT),2)  == 4.745074933325402533e9, tt::tolerance(1e-5));
    BOOST_TEST(pow(DeltaSpec(1.010157135208153312e+00/1e3, DELTA_TOT),2) ==  1.15292e-02*1e9, tt::tolerance(1e-5));
    //Check that it gives reasonable results when interpolating
    int k;
    for (k = 1; k < 100; k++) {
        double newk = 0.10022E+01/1e3+ k*(0.10362E+01-0.10022E+01)/1e3/100;
        BOOST_TEST(DeltaSpec(newk,DELTA_TOT) < DeltaSpec(0.10022E+01/1e3,DELTA_TOT));
        BOOST_TEST(DeltaSpec(newk,DELTA_TOT) > DeltaSpec(0.10362E+01/1e3,DELTA_TOT));
        BOOST_TEST(DeltaSpec(newk,DELTA_BAR)/DeltaSpec(0.10362E+01/1e3,DELTA_CDM) < 1);
        /*Check that the CDM + baryon power is the same as the total power for massless neutrinos*/
        BOOST_TEST(DeltaSpec(newk,DELTA_CB) == DeltaSpec(newk,DELTA_TOT), tt::tolerance(1e-5));
    }
    //Now check transfer functions: ratio of total to species should be ratio of T_s to T_tot squared: large scales where T~ 1
    //CDM
    BOOST_TEST(DeltaSpec(2.005305808001081169e-03/1e3,DELTA_CDM)/DeltaSpec(2.005305808001081169e-03/1e3,DELTA_TOT) == 1.193460280018762132e+05/1.193185119820504624e+05, tt::tolerance(1e-5));
    //Small scales where there are differences
    //T_tot=0.255697E+06
    //Baryons
    BOOST_TEST(DeltaSpec(1.079260830861467901e-01/1e3,DELTA_BAR)/DeltaSpec(1.079260830861467901e-01/1e3,DELTA_CB) == 9.735695830700024089e+03/1.394199788775037632e+04, tt::tolerance(1e-4));
    //CDM
    BOOST_TEST(DeltaSpec(1.079260830861467901e-01/1e3,DELTA_CDM)/DeltaSpec(1.079260830861467901e-01/1e3,DELTA_CB) == 1.477251880454670209e+04/1.394199788775037632e+04, tt::tolerance(1e-4));
}
...
/*         message(1,"k=%g G = %g F = %g G0 = %g\n",newk*1e3,dlogGrowth(newk, DELTA_TOT), F_Omega(0.01),dlogGrowth(newk, 1)); */
        ...
        BOOST_TEST(dlogGrowth(newk,DELTA_TOT) == (F_Omega(&CP, 0.01) * DeltaSpec(newk, DELTA_TOT)), tt::tolerance(1e-2));
        //Growth of CDM should be lower, growth of baryons should be higher.
        BOOST_TEST(dlogGrowth(newk,DELTA_CDM) < F_Omega(&CP, 0.01) * DeltaSpec(newk, DELTA_CDM));
        BOOST_TEST(dlogGrowth(newk,DELTA_CDM) / DeltaSpec(newk, DELTA_CDM) == 0.9389, tt::tolerance(1e-2));
        BOOST_TEST(dlogGrowth(newk,DELTA_BAR) > 1.25 * DeltaSpec(newk, DELTA_BAR));
        BOOST_TEST(dlogGrowth(newk,DELTA_BAR) < 1.35 * DeltaSpec(newk, DELTA_BAR));
    }
...
/*         message(1,"k=%g G = %g F = %g\n",newk*1e3,dlogGrowth(newk, 7), dlogGrowth(newk, 1)); */
        ...
        BOOST_TEST(dlogGrowth(newk,DELTA_TOT) < 1.055 * DeltaSpec(newk, DELTA_TOT));
        BOOST_TEST(dlogGrowth(newk,DELTA_TOT) > 1. * DeltaSpec(newk, DELTA_TOT));
        //CDM and baryons should match total
        BOOST_TEST(dlogGrowth(newk,DELTA_BAR) == dlogGrowth(newk,DELTA_TOT), tt::tolerance(0.008));
        BOOST_TEST(dlogGrowth(newk,DELTA_CDM) == dlogGrowth(newk,DELTA_TOT), tt::tolerance(0.008));
    }
}
...
BOOST_AUTO_TEST_CASE(test_read_rescale_sigma8)
{
    ...
    BOOST_TEST(nentry == 347);
    BOOST_TEST(pow(DeltaSpec(1.124995061548053968e-02/1e3, DELTA_TOT),2)* 4  == 4.745074933325402533e9, tt::tolerance(1e-2));
}
