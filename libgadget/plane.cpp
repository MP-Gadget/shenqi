...
plane_density_transfer(PetaPM * pm, int64_t k2, int kpos[3], pfft_complex * value)
{
    ...
    if(PlanePMCP && PlanePMCP->MassiveNuLinRespOn) {
        ...
        const double nufac = 1 + ps->nu_prefac * (*ps->nu_spline)(logk);
        value[0][0] *= nufac;
        value[0][1] *= nufac;
    }
}
        ...
        Cosmology * CP, const double atime, const double TimeIC, const double UnitLength_in_cm)
{
    ...
    pfft_complex * corrected_k = (pfft_complex *) mymalloc2("PlaneCorrectedK", double, grid->pm.priv->fftsize);
    ...
    grid->real = mymalloc("PlanePMRealOut", double, grid->pm.priv->fftsize);
        ...
        const double center, const double thickness, const double L)
{
    ...
    for(int64_t p = 0; p < PartManager->NumPart; p++) {
        if(!plane_particle_is_active(CP, atime, p))
            continue;
        const double pos = plane_wrap_position(Part[p].Pos[normal] - PartManager->CurrentParticleOffset[normal], L);
...
void write_plane(int snapnum, const double atime, const double TimeIC, Cosmology * CP, const std::string OutputDir, const double UnitVelocity_in_cm_per_s, const double UnitLength_in_cm)
{
    ...
    for (size_t i = 0; i < PlaneParams.CutPoints.size(); i++) {
        for (int j = 0; j < PlaneParams.Normals.size(); j++) {
            ...
            if(PlaneParams.Use3DMesh)
                num_particles_plane = cutPlanePMGrid(&plane_pm_grid, comoving_distance, BoxSize, CP, atime, PlaneParams.Normals[j], PlaneParams.CutPoints[i], PlaneParams.Thickness, plane_result);
            else
