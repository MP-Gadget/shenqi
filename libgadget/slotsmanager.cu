...
/* Initialize a new slot with type at index pi
 ...
 *
 * Assumes the particle is protected by locks in threaded environments.
 *
...
slots_convert(int parent, int ptype, int placement, struct part_manager_type * pman, struct slots_manager_type * sman)
{
    ...
    if(SLOTS_ENABLED(ptype, sman)) {
        int newPI = placement;
        /* If enabled, allocate a new slot for secondary data */
        if(placement < 0)
            newPI = atomic_fetch_and_add_64(&sman->info[ptype].size, 1);

        /* There is no clear way to safely grow the slots during this, because the memory may be deep in the heap.*/
...
{
    int i, nextgc = used;
    /* Find another non-garbage particle */
    for(i = start; i < used; i++)
        if(!GARBAGE(i, ptype, pman, sman)) {
            nextgc = i;
            break;
...
slots_gc_base(struct part_manager_type * pman)
{
     ...
     * ReverseLink is valid only within gc.
     */
...
static int slot_cmp_reverse_link(const void * b1in, const void * b2in) {
    const struct particle_data_ext * b1 = (const struct particle_data_ext *) b1in;
    const struct particle_data_ext * b2 = (const struct particle_data_ext *) b2in;
...
#pragma omp parallel for
    for(i = 0; i < sman->info[ptype].size; i++) {

#ifdef DEBUG
        if(BASESLOT_PI(i, ptype, sman)->ReverseLink >= pman->MaxPart) {
            endrun(1, "Should not happen: i=%ld ptype=%d\n", i, ptype);
        }
#endif
 ...
 * This does a garbage collection by sorting the garbage to the end of the array and then trimming.
...
void slots_gc_sorted(struct part_manager_type *pman, struct slots_manager_type *sman) {
    ...
    for (i = 0; i < pman->NumPart; i++) {
        ...
        // This is to avoid using a struct with two uint32_t members. Which is
        // not supported by cub::DeviceRadixSort::SortPairs.
...
/* Mark the i-th base particle as garbage. */
void
...
slots_check_id_consistency(struct part_manager_type * pman, struct slots_manager_type * sman)
{
    ...
    for(i = 0; i < pman->NumPart; i++) {
        ...
        if(PI >= info.size) {
            endrun(1, "Slot PI consistency failed\n");
        }
...
/* This function needs the Type of P[i] to be set up */
void
