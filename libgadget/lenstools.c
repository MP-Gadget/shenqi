...
double *allocate_3d_array_as_1d(int Nx, int Ny, int Nz) {
    double *array = mymalloc("3d", double, Nx * Ny * Nz);
...
double *allocate_2d_array_as_1d(int Nx, int Ny) {
    double *array = mymalloc("2d", double, Nx * Ny);
    // Initialize elements with memset
...
int64_t cutPlaneGaussianGrid(int64_t num_particles_tot, double comoving_distance, double Lbox, const Cosmology * CP, const double atime, const int normal, const double center, const double thickness, const double *left_corner, const int plane_resolution, double *lensing_potential) {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        binning[i] = mymalloc("lensbins", double, resolution + 1);
    ...
    grid3d_ngb(PartManager->Base, num_particles_rank, binning, dims, CP, atime, density);
...
#ifdef USE_CFITSIO
void savePotentialPlane(double *data, int rows, int cols, const std::string filename, double Lbox, Cosmology * CP, double redshift, double comoving_distance, int64_t num_particles, const double UnitLength_in_cm, const int plane_double) {
