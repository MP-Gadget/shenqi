...
static int _compar_radix_u8(const void * r1, const void * r2, size_t rsize) {
...
static void _bisect_radix(void * r, const void * r1, const void * r2, size_t rsize) {
    size_t i;
...
void _setup_radix_sort(
    ...
    switch(rsize) {
        ...
        case 8:
            ...
            break;
        default:
            d->compar = _compar_radix_u8;
            d->bisect = _bisect_radix;
    }
}
...
static void radix_sort_bytes(void * base, size_t nmemb, size_t size,
    ...
    std::sort(indices.begin(), indices.end(), [keys, rsize](size_t a, size_t b) {
        return _compar_radix_u8(keys + a * rsize, keys + b * rsize, rsize) < 0;
    });
    myfree(keys);
    /* Rearrange the data into a temporary buffer according to indices,
     * then memcpy back to base. */
